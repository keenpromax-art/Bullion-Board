...
- Workspace grid: `1.3fr 1fr / 1fr 1fr` 4-up default, `6px` gap + padding. Layouts `1-up | 2-up | 2-H | 3-R | 3-L | 4-up` switch via status-bar select; `+ PANEL` adds a directory panel and focuses the command line. Nested split panes (shared left/right column width + independent top/bottom height per column, so all four quadrants resize separately) via amber dividers — drag with mouse or focus + arrow keys (Shift = big step); splits persist per layout, clamped 20–80%. Hard cap of 4 panels (`MAX_PANELS` in `workspaceStore.ts`): `NEW`/Shift+Enter/MENU-new/deep links reuse the focused panel at cap; duplicate/detach/`+ PANEL` are blocked with a notice; saved/imported workspaces trim to 4.
...
- Status bar: 26px, 10.5px gray uppercase segments — `YAHOO FEED · LIVE | N FUNC | M PANELS OPEN | WORKSPACE: <name> (click = save/load/rename/delete/export/import layouts) | DESKS: <chips, click = switch · double-click = rename · right-click = close · + = new> | FOCUS: FNC · DESK | layout select | + PANEL | ⚙ SET | ? TOUR | clock IST`. Virtual desktops (Linux-style, max 6 via `MAX_DESKTOPS`): each holds its own panels/layout/focus/splits, all autosaved together (key `bb.workspace.shell.v1`, migrates legacy `bb.workspace.active.v2` into desk 1); dirty `*` until saved. Default preset `MARKET OVERVIEW` ships a one-glance starter dashboard (macro board 111 + function menu DIR + Reliance technicals 1 + news wire 47). Full desks keep internal scroll only as an overflow fallback.
- First run: fresh browsers (no saved shell, tour never dismissed — key `bb.tour.seen.v1`) get a 7-step guided tour over the starter dashboard — command line, tape, panels, F-keys, status bar, dashboard — with an amber spotlight ring per step; SKIP/←/→/Esc supported, `? TOUR` in the status bar replays it anytime.
