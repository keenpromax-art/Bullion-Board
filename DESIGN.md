...
- Status bar: 26px, 10.5px gray uppercase segments — `YAHOO FEED · LIVE | N FUNC | M PANELS OPEN | WORKSPACE: <name> (click = save/load/rename/delete/export/import layouts) | FOCUS: FNC · DESK | layout select | + PANEL | clock IST`. Active layout autosaves; dirty `*` until saved. Default preset `EQUITY OVERVIEW` so first run is populated.
