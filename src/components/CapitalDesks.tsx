    ...
    <div className="grid">
      <div className="panel panel-glow">
        ...
        <div className="cells">
          ...
          <div className="cell"><div className="lbl">Target range</div><div className="val" style={{ fontSize: 14, whiteSpace: "nowrap" }}>{tg.low && tg.high ? `${rs(tg.low)}–${rs(tg.high)}` : "—"}</div><div className="sub">{tg.dispersionPct !== null && tg.dispersionPct !== undefined ? <span data-explain="Dispersion">disp {tg.dispersionPct}%</span> : `med ${tg.median ? rs(tg.median) : "—"}`}</div></div>
          ...
          <div className="cell"><div className="lbl">Fwd PE / Trail</div><div className="val">{s.forwardPE?.toFixed(1) ?? "—"} / {s.trailingPE?.toFixed(1) ?? "—"}</div><div className="sub"><span data-explain="PEG">PEG {peg !== null && isFinite(peg) ? peg.toFixed(2) : "—"}</span></div></div>
